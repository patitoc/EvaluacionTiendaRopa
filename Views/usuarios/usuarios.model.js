...
class Usuarios_Model {
  ...
  eliminar() {
    console.log('usuario id a eliminar',this.UsuarioId)
    var UsuarioId = this.UsuarioId;
  ...
  editar() {
    ...
    $.ajax({
      ...
      success: function (res) {
        res = JSON.parse(res);
        if (res === "ok") {
          Swal.fire("usuarios", "Usuario Actualizado", "success");
          todos_controlador();
        } else {
          Swal.fire("Error", res, "error");
        }
  ...
  cedula_repetida(){
    ...
    $.post("../../Controllers/usuario.controller.php?op=cedula_repetida", {Cedula: Cedula}, (res) => {
        ...
        if( parseInt(res.cedula_repetida) > 0){
            ...
            $('#CedulaRepetida').html('La cédula ingresada, ya exite en la base de datos');
